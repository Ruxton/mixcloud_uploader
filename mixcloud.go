...
func main() {
  ...
  if(*configFlag == true) { createConfig() }

  if(*trackListFlag != "") { tracklist = parseTrackList(trackListFlag) }

  if(*fileFlag == "") { OutputError("You must pass a file to upload, use --file or see --help.\n Exiting."); os.Exit(2); }
...
func HttpUploadRequest(file string, cover string, name string, desc string, tag_list []string, tracklist []Track) (*http.Request,*pb.ProgressBar) {
  ...
  // Add MP3
  if file != "" { loadFileToWriter(file,"mp3",writer) }

  // Add cover image
  if cover != "" { loadFileToWriter(cover,"picture",writer) }
